...
export default function RecordWithAIModal({ onClose }: Props) {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: "Hi! I'm your AI Assistant. You can type your transactions or upload a receipt photo. (Voice feature is disabled in this mock-up).",
    },
  ]);
  ...
  async function send() {
    ...
    try {
      ...
      if (data.parsed) {
        setMessages((prev) => [...prev, { role: "assistant", content: "✅ Transaction parsed! Opening form..." }]);
        setPrefill(data.parsed);
        setTimeout(() => setShowManual(true), 800);
      } else {
        setMessages((prev) => [...prev, { role: "assistant", content: data.reply ?? "I couldn't understand that." }]);
      }
    } catch {
      ...
      <div className="w-full sm:max-w-md bg-[#161b22] rounded-t-2xl sm:rounded-2xl border border-[#30363d] shadow-2xl flex flex-col h-[85vh] sm:h-[600px]">
        <div className="flex items-center justify-between p-4 border-b border-[#30363d]">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[#e6edf3] font-semibold text-sm">AI Assistant (Mock-up)</h2>
          </div>
